(()=>{
  ...
  function mount(){
    ...
    wrap.innerHTML=`<h3>Endereço da House</h3><p class="address-hint">Rua/Av, nº, bairro, CEP e Cidade/UF.</p><form id="addressForm" class="address-form"><div class="address-line"><input id="addrStreet" placeholder="Rua/Av, nº" value="${esc(p.address_line||'')}"><input id="addrNeighborhood" placeholder="Bairro" value="${esc(p.neighborhood||'')}"><input id="addrPostal" placeholder="CEP" value="${esc(p.postal_code||'')}"><input id="addrCity" placeholder="Cidade" value="${esc(p.city||'')}"><input id="addrState" placeholder="UF" maxlength="2" value="${esc(p.state||'SC')}"><select id="meetingDay" aria-label="Dia fixo do encontro"><option value="">Dia do encontro</option><option value="0" ${String(p.meeting_day??'')==='0'?'selected':''}>Domingo</option><option value="1" ${String(p.meeting_day??'')==='1'?'selected':''}>Segunda-feira</option><option value="2" ${String(p.meeting_day??'')==='2'?'selected':''}>Terça-feira</option><option value="3" ${String(p.meeting_day??'')==='3'?'selected':''}>Quarta-feira</option><option value="4" ${String(p.meeting_day??'')==='4'?'selected':''}>Quinta-feira</option><option value="5" ${String(p.meeting_day??'')==='5'?'selected':''}>Sexta-feira</option><option value="6" ${String(p.meeting_day??'')==='6'?'selected':''}>Sábado</option></select><button type="submit" class="address-save">Salvar</button></div><p id="addressMsg" class="address-msg"></p></form><div class="route-actions"><button type="button" id="openWaze">Abrir no Waze</button><button type="button" id="openMaps">Abrir no Google Maps</button></div>`;
    host.insertAdjacentElement('afterend',wrap);
    $('#addressForm').onsubmit=async e=>{e.preventDefault();const btn=e.submitter,msg=$('#addressMsg');btn.disabled=true;msg.textContent='Salvando…';try{const body={action:'update_address',address_line:$('#addrStreet').value,neighborhood:$('#addrNeighborhood').value,postal_code:$('#addrPostal').value,city:$('#addrCity').value,state:$('#addrState').value,meeting_day:$('#meetingDay').value};const r=await fetch(endpoint(),{method:'POST',headers:{'Content-Type':'application/json'},body:JSON.stringify(body)});const j=await r.json();if(!r.ok)throw Error(j.error||'Não foi possível salvar');if(j.house_profile)data.house_profile={...data.house_profile,...j.house_profile};if(typeof render==='function')render();msg.textContent='Endereço atualizado.';setTimeout(()=>msg.textContent='',2500)}catch(err){msg.textContent=err.message}finally{btn.disabled=false}};
