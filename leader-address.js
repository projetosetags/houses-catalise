(()=>{
  ...
  function profile(){return window.data?.house_profile||null}
  function fullAddress(p){return [p?.address_line,p?.neighborhood,p?.postal_code,p?.city&&p?.state?`${p.city}/${p.state}`:p?.city||p?.state].filter(Boolean).join(', ')}
